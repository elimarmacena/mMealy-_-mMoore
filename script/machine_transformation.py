...
def check_kind_machine(machine : dict) -> int:
	"""
	Return 1, for Moore machine, 2, for Mealy machine.

	:param machine: Dictionary that represents a machine
	:return: 1 if it's a Moore machine, 2, if it's Mealy's
	"""
...
def __list_validation(lst:list)->bool:
...
def list_to_machine(lst_machine: list)->dict:
	if __list_validation(lst_machine):
		machine = {}
...
def __moore_intransitive(moore: dict)->bool:
	starts = moore["start"]
...
def __treat_transaction_moore(moore: dict):
	mealy_transacion = []
...
def __treat_final_symbols(new_states: list, finals_mealy: list):
	new_finals_states = []
...
# USED TO CHECK IF A OUTPUT SYMBOL IS ALREADY REGISTRED (ASSIST FUNCTIO TO __treat_states)
def __check_output(symbol_output: str, created_output: list)->bool:
	for output in created_output:
		if output[1] == symbol_output:
			return True
	return False

# USED TO CREATE A ITEM TO BE ADD IN THE STRUCT OF THE FUNCTION __treat_states
def __states_outfn(state: str, out_fn: list)->list:
	__states_outfn = [state, []]
	for data in out_fn:
		__states_outfn[1].append(data)
	return __states_outfn
...
def __transform_transaction_to_moore(transaction: list, new_states: list):
	moore_transactions = []
	for state_created in new_states:
...
def mealy_to_moore(mealy: dict)->dict:
	if (check_kind_machine(mealy) == 2):
		new_states = __treat_states(mealy)  # NEW STATES WITH THE STATE ORIGEM
		...
		moore["final"].extend(__treat_final_symbols(new_states, mealy["final"]))
		...
		for mealy_transacions in mealy["trans"]:
			new_destine_state = __treat_out_transaction(
			...
			moore_transactions = __transform_transaction_to_moore(
